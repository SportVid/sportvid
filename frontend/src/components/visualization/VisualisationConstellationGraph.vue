<template>
  <v-row>
    <v-col cols="3">
      <v-btn
        class="my-3 d-print-block mx-auto"
        :disabled="loading"
        @click="renderGraph"
      ...
      <div style="max-height: 700px" class="overflow-y-auto">
        ...
        <v-list dense>
          <v-list-item v-for="(cluster, id) in clusterSettings" :key="id">
            ...
            <v-list-item-content>
              <v-list-item-title :class="{ 'grey--text': !cluster.active }">{{
                cluster.name
              }}</v-list-item-title>
            </v-list-item-content>
            <v-list-item-action>
        ...
        <v-list>
          ...
          >
            ...
            <v-list-item-content>
              <v-list-item-title :class="{ 'grey--text': !timeline.active }">{{
                timeline.name
              }}</v-list-item-title>
            </v-list-item-content>
            <v-list-item-action>
            ...
            <VisualizationGraph
              v-if="timeline.showPlot"
              :threshold="timeline.threshold"
              showLegend="false"
              plotType="scatter"
              :timelineId="timeline.id"
              style="max-width: 20vw; max-height: 400px"
            >
            </VisualizationGraph>
          </v-list-item>
        </v-list>
    ...
    <v-col cols="9" style="position: relative">
      <div
        ref="visualizationTimelineConstellationGraph"
        style="min-height: 500px; height: 95%"
      ></div>
      <div
        v-if="loading"
...
export default {
  ...
  methods: {
    ...
    calcTimelineOverlap(timestamps1, timestamps2) {
      if (this.shot_aggregation) {
        return this.shotStore.shots
          .map(
            (s) =>
              timestamps1.filter((t) => s.start <= t && t <= s.end).length > 0 &&
    ...
    countAppearance(timestamps) {
      if (this.shot_aggregation) {
        return this.shotStore.shots
          .map((s) => timestamps.filter((t) => s.start <= t && t <= s.end))
          .filter((ys) => ys.length > 0).length;
      } else {
        // frame based
    ...
    getConstellations() {
      const activeTimelines = Object.values(this.timelineSettings)
        ...
        .map((tSetting) => {
          ...
          tSetting.timestamps = timeline.plugin.data.time.filter(
            (time, index) => timeline.plugin.data.y[index] >= tSetting.threshold
          );
          return tSetting;
        });
      ...
      const edges = new DataSet(
        node_combinations
          .map((c) => {
            const overlap = this.calcTimelineOverlap(c[0].timestamps, c[1].timestamps);
            return {
              from: c[0].id,
              to: c[1].id,
              id: c[0].id + c[1].id,
  ...
  computed: {
    timelines() {
      return this.timelineStore.all.filter(
        (timeline) =>
          timeline.type === "PLUGIN_RESULT" && timeline.plugin && timeline.plugin.type === "SCALAR"
      );
    },
    visibleTimelines() {
