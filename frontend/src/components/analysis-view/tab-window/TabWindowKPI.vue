<template>
  ...
  <div
    v-else-if="!hasKpiData && (visualizationStore.isLoadingKpi || kpiComputationActive)"
    class="kpi-loading-card"
  >
    <div class="kpi-spinner"><i class="mdi mdi-loading mdi-spin" /></div>
    <div v-if="kpiComputationActive" class="kpi-loading-text">{{ kpiComputationProgressText }}</div>
  </div>

  <div
    v-else-if="!hasKpiData"
    class="d-flex flex-column align-center"
    style="gap: 12px; margin-top: 4vh"
    data-tour="kpi-compute-card"
  >
    <div
      v-if="kpiEmptyStateText"
      class="text-h6 text-grey font-weight-light mx-16 px-10 text-center"
      style="line-height: 1.5"
      v-html="kpiEmptyStateText"
    />

    <v-btn
      class="mt-4"
      v-if="canWrite"
      :disabled="!hasAvailablePositionData"
      data-tour="kpi-open-compute"
      @click="showKpiComputationModal = true"
    >
      {{ $t("visualization.kpi.compute_kpis") }}
    </v-btn>
    <v-btn :disabled="!hasDoneKpiRun" @click="showModalPositionDataSelect = true" class="my-4">
      {{ $t("visualization.kpi.select_kpis") }}
    </v-btn>
  </div>
  ...
  <v-card v-else class="d-flex flex-column flex-nowrap px-2 mb-1" elevation="0">
    <v-row align="center" class="flex-nowrap" data-tour="kpi-controls-row">
      <v-col
        cols="auto"
        class="mt-3 d-flex align-center flex-shrink-0"
        style="gap: 8px"
                      ...
                      <v-list-item class="menu-item" @click.stop>
                        <v-list-item-title class="d-flex align-center justify-space-between">
                          ...
                          <v-btn-toggle
                            ...
                            style="height: 22px"
                          >
                            <v-btn value="ms" size="x-small" style="font-size: 11px; padding: 0 6px"
                              >m/s</v-btn
                            >
                            <v-btn
                              value="kmh"
                  ...
                  <v-list-item-title class="d-flex align-center">
                    ...
                    >
                      <v-btn value="ms" size="x-small" style="font-size: 10px; padding: 0 4px"
                        >m/s</v-btn
                      >
                      <v-btn value="kmh" size="x-small" style="font-size: 10px; padding: 0 4px"
                        >km/h</v-btn
                      >
                    </v-btn-toggle>
                    <tab-window-icon
    ...
    <div v-if="viewMode === 'table'" data-tour="kpi-table-view">
      <div v-if="groupMode === 'player'" class="team-tables d-flex flex-wrap justify-space-around">
        <v-card
      ...
      <div v-else class="team-tables d-flex flex-wrap justify-space-around">
        <v-card
  ...
  <ModalPluginRun
    ...
    runButtonDataTour="kpi-compute-run"
  />
...
import { kpiComputationParams } from "@/composables/usePluginParams";
...
const kpiEmptyStateText = computed(() => {
  if (!hasAvailablePositionData.value) {
    const full = t("visualization.kpi.posdata_not_available");
    return canWrite.value ? full : full.split("<br>")[0];
  }
  if (!hasDoneKpiRun.value) {
    return t("visualization.kpi.no_kpi_computed");
  }
  return "";
});
...
const hasDoneKpiRun = computed(() => kpiRunsForVideo.value.some((e) => e.status === "DONE"));
