...
class TimelineVideoSampler(
    ...
    def call(
        ...
        with inputs["input"] as input_data, inputs["shots"] as shots_data, data_manager.create_data("ImagesData") as output_data:
            ...
            middle_time = (current_shot.end - current_shot.start)/2+ current_shot.start
            ...
            with input_data() as input_iterator:
                for frame in input_iterator:
                    ...
                    if parameters.get("start_frame") and time >= current_shot.start and abs(current_shot.start-time) <  start_closest:
                        start_closest = abs(current_shot.start-time)
                        start_frame = frame

                    if parameters.get("end_frame") and time <= current_shot.end and abs(current_shot.end-time) <  end_closest:
                        end_closest = abs(current_shot.end-time)
                        end_frame = frame

                    if parameters.get("middle_frame") and abs(middle_time-time) <  middle_closest:
            ...
            for image in result_images:
                ...
                output_data.save_image(
                    image.get("frame"), ext="jpg", time=image.get("time"), delta_time=1 / image.get("delta_time")
                )
