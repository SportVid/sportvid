...
class TrackingDataPosDataChunk(View):
    """
    Serves position data in paginated chunks.

    GET /tracking_data/pos_data/chunk
    ...
    Response:
        { status, frames: {timeMs: players[]}, total, offset, limit }
    ...
    def get(self, request):
        try:
            ...
            for pr in plugin_runs:
                for result in PluginRunResult.objects.filter(plugin_run=pr):
