...
@DataManager.export("KinexionPsData", analyser_pb2.IMAGE_EMBEDDING_DATA)
@dataclass(kw_only=True)
class KinexionPsData(Data):
    type: str = field(default="TimeNDEmbeddings")
    embeddings: List[TimeNDEmbedding] = field(default_factory=list)

    def load(self) -> None:
        super().load()
        assert self.check_fs(), "No filesystem handler installed"

        data = self.load_dict("time_nd_embedding.yml")
        self.embeddings = [TimeNDEmbedding(**x) for x in data.get("embeddings")]
