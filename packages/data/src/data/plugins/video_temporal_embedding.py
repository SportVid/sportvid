...
class VideoTemporalEmbedding(Data):
    image_id: str = None
    ref_id: str = None
    time: float = None
...
@DataManager.export("VideoTemporalEmbeddings", analyser_pb2.VIDEO_TEMPORAL_EMBEDDING_DATA)
@dataclass(kw_only=True)
class VideoTemporalEmbeddings(Data):
    type: str = field(default="VideoTemporalEmbeddings")
    embeddings: List[VideoTemporalEmbedding] = field(default_factory=list)

    def load(self) -> None:
        super().load()
        assert self.check_fs(), "No filesystem handler installed"

        data = self.load_dict("video_temporal_embeddings_data.yml")
        self.embeddings = [VideoTemporalEmbedding(**x) for x in data.get("embeddings")]
    ...
    def save(self) -> None:
        ...
        self.save_dict("video_temporal_embeddings_data.yml", {"embeddings": [x.to_save() for x in self.embeddings]})
